...
Given a Replicate model identifier and its required inputs, this skill creates a prediction via the Replicate API, waits for it to complete, and returns the artifact URL of the prediction output.
...
## Output
- `artifactUrl`: The URL of the prediction's primary output artifact.

## Examples
- Image generation: `{ model: "black-forest-labs/flux-schnell", input: { prompt: "a cat astronaut" } }` → returns the generated image URL
- Text generation: `{ model: "meta/meta-llama-3-70b-instruct", input: { prompt: "Explain quantum computing" } }` → returns the generated text
